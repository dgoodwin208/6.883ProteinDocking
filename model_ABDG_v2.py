...
class ProteinGCN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #tensor_size(atom_emb,'Atom Emb 1')
        #tensor_size(lookup_tensor,'Lookup Tensor')
        

        atom_mask = atom_mask.unsqueeze(dim=-1)
        tensor_size(atom_mask, 'Atom Mask')

        for idx in range(self.n_conv):
            atom_emb *= atom_mask
            atom_emb = self.convs[idx](atom_emb, nbr_emb, nbr_adj_list, atom_mask)
            #tensor_size(atom_emb,'Atom Emb 2')

        # Update the embedding using the mask
        atom_emb *= atom_mask

        # generate reside amino acid level embeddings
        amino_emb_raw, mask_pooled  = self.pooling_amino(atom_emb, atom_amino_idx)
        #tensor_size(amino_emb,'Amino Emb')
        amino_emb               = self.amino_to_fc(self.amino_to_fc_activation(amino_emb_raw))
...
#         out         = [self.fc_out(protein_emb), self.fc_amino_out(amino_emb), mask_pooled]
        ...
        return amino_emb_raw, amino_emb
