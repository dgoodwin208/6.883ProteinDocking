...
parser.add_argument('-parallel_jobs',       default=1,                                  help='Number of threads to use for parallel jobs')
...
for directory in protein_dirs:
	if "DS_Store" in directory:
		print(f"Skipping erroneous directory {directory}")
		continue
...
def runCommands(directory):
	...
	for filename in tqdm(rest_pdb_files):
		...
		#Removing the native.pdb file
		command = cpp_executable + ' -i ' + pdb_filepath + ' -j ' + json_filepath
		print(f"DAN PRINT: command:  {command}")
...
def processDirectory(directory, max_neighbors, savepath, protein_id_prop_file):
	...
	for filename in all_json_files:
		...
		with open(protein_id_prop_file, 'a') as file:
			...
			writer.writerow((save_filename, -1, -1)) #Quick fix to make it run
